...
function shapechange(evt) {
    ...
    tokens.forEach(token => {
        ...
        const src = rollableTableItemsObj[newSide]
            .get('avatar')
            .replace('/max', '/thumb');
        token.set('currentSide', newSide);
        token.set('imgsrc', src);
        const represents = token.get('represents');
        const type = getAttrByName(
            represents,
            'shapechanger_type'
        ).toLowerCase();
...
shapechange.lythari = (token, newSide, represents) => {
    ...
    if (!baseStrength)
        baseStrength = setSheetAttribute(
            represents,
            'lythari_base_strength',
            strength.get('current')
        );
    if (!baseSpeed)
        baseSpeed = setSheetAttribute(
            represents,
            'lythari_base_speed',
            speed.get('current')
        );
    if (!baseAC)
        baseAC = setSheetAttribute(
            represents,
            'lythari_base_ac',
            ac.get('current')
        );
